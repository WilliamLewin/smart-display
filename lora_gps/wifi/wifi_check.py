...
def getCurrentWifi():
    os.system('sudo iwconfig | grep wlan0 >> /home/pi/lora_gps/wifi/currentwifi.txt')
    file = open('/home/pi/lora_gps/wifi/currentwifi.txt', 'r')
    wifi = file.read()
    buffer = []
...
def wifiScan():
    os.system("sudo iwlist wlan0 scan | grep ESSID | awk '{print substr($0, index($0,$1)) }' >> /home/pi/lora_gps/wifi/wifi.txt")
    file = open('/home/pi/lora_gps/wifi/wifi.txt','r')
    wifi = file.read()
    buffer = []
    for i in range(0,len(wifi)):
        if wifi[i] != '\n':
...
if 'CCGuest' in trimmedWifi:
    ...
    if 'SmartWorkplaceIphone' in buffer:
        ...
        os.system('sudo /home/pi/lora_gps/wifi/wifi-ccguest.sh')
else:
    print("Done")
